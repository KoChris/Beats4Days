...
require([
        ...
        "dojo/data/ItemFileWriteStore"
        		...
        		domConstruct, domGeom, CheckBox, Dialog, MenuItem, Menu, DropDownButton, popup, toaster,io_script){
	...
	var isEnvCordovaCapable = function(){
		...
		if (smartPreview == "On") // if smart preview is on, always show cordova sim panel
		{
			return true;
		}
	...
	/** the default root webpage of the application to display in the webpage box */
	var rootWebpage = null;

	/** the default platform */
	var defaultPlatform = null;
	
	/** smart preview boolean */
	var smartPreview = null;
	
	/** showSingleEnvironment string */
	var showSingleEnvironment = null;
	...
	/** the environmentsStore */
	var environmentsStore = null;
	...
	var createGrid = function() {
		...
		grid = new ibm_mobile.MobileBrowserSimulatorContainer({
		    ...
		    'smartPreview' : smartPreview,
		    'showSingleEnvironment' : showSingleEnvironment,
	...
	var deviceLoad = function(response, ioArgs){
		    ...
		    //if smart preview is on, only load existing environment platforms
		    if(smartPreview == "On")
		    {

		    	var resolutionItems = {};
                var blackberryPlatformInStore = false;
                environmentsStore.fetch({ //fetch environments
				    'start': 0,
				    'onItem' : function(environment) {
						var platformID =  "platform." + environment.name;//set platform id from environments store
						if(platformID == "platform.iphone")
						{
							platformID = "platform." + "ios.iphone";
						}
						if(platformID == "platform.ipad")
						{
							platformID = "platform." + "ios.ipad";
						}
						if(platformID == "platform.windowsphone8")
						{
							platformID = "platform." + "windowsphone";
						}
						if(platformID == "platform.blackberry10")
						{
							platformID = "platform." + "blackberry"; 
						}
                                                if( (platformID == "platform.blackberry") && (blackberryPlatformInStore == true)) //only put blackberry platform into storeData once
                                                {          
							platformID = "skip";
						}

                                                
			     		dojo.forEach(response['platforms'], function(platform){ //fetch platforms from device list
				    		 if ((platform.id == platformID) || (platformID == null)) { //if platform id is in the environments list
						      // create platform
						      // item
                             ...
                             if(platform.id == "platform.blackberry") //only put blackberry platform into storeData once
                             {
                                    if(blackberryPlatformInStore == false )
                                    {
                                    	// push to store
						                storeData.items.push(platforms[platform.id]);
					                    blackberryPlatformInStore = true;
                                    }
 						    }
						    else
						    {
			    		  ...
			    		  dojo.forEach(response['devices'], function(device){
			    		    	if ((platformID == device.platformID) || (platformID == null)) {
			    				    ...
			    				    var agentID_Worklight = device.agentID + "/Worklight/6.2";
			    				    var deviceItem = {
			    				        "type" : "device",
			    				        "id" : device.name,
			    				        "name" : device.name,
			    				        "agentID" : agentID_Worklight,
				    ...
				    'onError' : function(error) {
					    console.error("Error while trying to load environment from environment list object. "  + error);
				    },
				    'scope': this
			    });	
             
		    }
		    else //non smart preview load platforms
		    {
		...
		var skinsLoad = function(response, ioArgs){
		    // create parent store
		    var skinsStoreData = {
			        "label" : "name",
			        "identifier" : "name",
			        "items" : [ {   "name":'default', "platform":'all' } ]
			    };
		    
			var skinsObject = {};
		    dojo.forEach(response['skins'], function(skins){
		    	//if ((platform.id == platformID) || (platformID == null)) { // on app folder preview check for platform match in mbs.js
			    // create skins item
		    	if(skins.platform == "blackberry10")
		    	{
		    		skins.platform = "blackberry";
		    	}
		    	skinsObject = {
				        "name" : skins.name,
				        "platform" :skins.platform

				    };
				    // push to store
				    skinsStoreData.items.push(skinsObject);
				    
		    	//}
		    }); 
		    
	        // create read store from data
		    skinsStore = new dojo.data.ItemFileReadStore({
			    data : skinsStoreData
		    });
	...
	var onEnvironmentStoreReady = function(devicesFilePath) {

		// get devices json and build device store for smart preview case then filter by environments
...
//				loadScript("/OpenLayers/OpenLayers-2.12/OpenLayers.js", false,
//				function(){
					require(["dijit/TitlePane", 
							 "dijit/form/HorizontalSlider", 
							 "dijit/form/HorizontalRule", 
...
//				initWithoutMap);
 ...
 /*	var wireQRCode = function() {
	...
	ready(function(){		
		...
		/** the file containing the skin profiles */
		var skinsFilePath = "defaultSkinslist.json";
		
		/** the file containing the environment list */
		var environmentFilePath = "environmentList.json";
		...
		if (window.location.search) {
			...
			if (quary['skinsFilePath']) {
				skinsFilePath = quary['skinsFilePath'];
			}

			if (quary['webpage']) {
		        //set default webpage
				defaultWebpage = quary['webpage'];
				var urlList = defaultWebpage.split('/'); 
                //catch console case for Portal webpage mainFile case 						 
				var mainFile = urlList.slice(11); 
				urlList = urlList.slice(0,8); 	//calculate root webpage for Go/Refresh box
                rootWebpage = urlList.join("/"); 
				dijit.byId('_webpageBox').set('value', rootWebpage);				
				//check for Portal case of hybrid app that points to a webpage
				var httpCount = defaultWebpage.match(/http/g);  
				if(httpCount == null) { //console case if http count is null				
					httpCount = 0;
				}				
				if( httpCount.length > 1 && (mainFile.toString().substring(0,4) == "http") && (mainFile.toString().indexOf(":") != -1 )   ) {
                     var portalCase = true; //set Portal case to true, TODO Get rid of cross domain block and try to tool for Cordova
                     dijit.byId('_webpageBox').set('value', mainFile.join("/") );
					 defaultWebpage = mainFile.join("/");
				}								
			...
			if (quary['smartPreview'])
			{
				smartPreview = quary['smartPreview'];
			}
			else
			{
				smartPreview = "Off";
			...
			if (quary['showSingleEnvironment'])
			{
				showSingleEnvironment = quary['showSingleEnvironment'];
			}
			else
			{
				showSingleEnvironment = "Off";
			}
		...
		dojo.connect(dijit.byId('_goBtn'), 'onClick', function(){
			var box = dijit.byId('_webpageBox');
            var webpageForDevice = box.get('value'); 
            //if webpage url is the root url, send defaultWebpage with all parameters needed by each device frame
            if(box.get('value') == rootWebpage)
            {
                webpageForDevice = defaultWebpage;
            }
			connect.publish('/mbs/webpage', [{
				'webpage' : webpageForDevice
			}]);
		});
	    ...
	    var environmentLoad = function(response, ioArgs){
		    // create parent store
		    var environmentStoreData = {
			        "label" : "name",
			        "identifier" : "name",
			        "items" : [ ]
			    };
		    ...
		    dojo.forEach(response['environments'], function(environments){
		    	environmentObject = {
				        "name" : environments.name
				    };
				    // push to store
		    	environmentStoreData.items.push(environmentObject);				    
		    }); 
		    
	        // create read store from data
		    environmentsStore = new dojo.data.ItemFileReadStore({
			    data : environmentStoreData
		    });
		...
		dojo.xhrGet({
			...
			load : function(data) {
				if (data && data.pollUrl) {
					// Set up the long poll using data.pollUrl
                    var poll = function() {
                        io_script.get({
                            url: data.pollUrl,
                            callbackParamName: "callback"
                        }).then(function(data) {
                        	// Data received - pop up the toaster
                        	if (domStyle.get("message_toaster", "display") === "none") {
	                            var toaster = dijit.byId("message_toaster");
	                            toaster.setContent(data.message, 'message');
	                            dojo.style(toaster.contentNode, "background", "#fff0b4");
	                            dojo.style(toaster.contentNode, "font-weight", "normal");
	                            dojo.style(toaster.contentNode, "fontWeight", "normal");
	                            dojo.style(toaster.contentNode, "color", "black");
		...
		// build the skins store
		var skinsStoreDeferred = dojo.xhrGet({
		    url : skinsFilePath, //needs to be skins file path, which is generated from eclipse preview
		    handleAs : "json",
		    preventCache : true,
		    load : skinsLoad,
		    error : function(response, ioArgs){
			    console.error("Could not retreive skins listing from '" + this.url + "': " + response);
			    var skinsStoreData = {
				        "identifier": "name",
				        "label": "name",
				        "items": [
				            { "name": "default", "platform": "all" }   ]
				    }; 			   	
			    ...
			    // add items dependent on dataStore
			    // skinsStoreDeferred.addCallback(onDeviceStoreReady);
			    return response;
		    }
		});
		
		// build the environment store if smart Preview is on
	    if(smartPreview == "On")
        {		  
		  var environmentStoreDeferred = dojo.xhrGet({
		    url : environmentFilePath, //environment file path, which is generated from eclipse preview
		    handleAs : "json",
		    preventCache : true,
		    failOk : true,
		    load : environmentLoad,
		    error : function(response, ioArgs){
			    console.error("Could not retreive environments listing from '" + this.url + "': " + response);
			    var environmentStoreData = {
				        "identifier": "name",
				        "label": "name",
				        "items": [   ]
			     ...
			     // create read store from data
			   	environmentStore = new dojo.data.ItemFileReadStore({
			   		   data : environmentStoreData
			   	});			   		    
			    // add items dependent on dataStore
			     environmentStoreDeferred.addCallback(onEnvironmentStoreReady(devicesFilePath));
			    return response;
		    }
		 });
                    environmentStoreDeferred.addCallback(onEnvironmentStoreReady(devicesFilePath));
        }	
	    else //else just build the device store
	    {
